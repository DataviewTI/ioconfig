new IOService({
  ...
  function(self){
    ...
    $('#odin-cpf_cnpj').mask($.jMaskGlobals.CPFCNPJMaskBehavior,{
      onKeyPress: function(val, e, field, options){
          ...
          args.push(iscpf=>{
            if(self.fv!==null){
              if(iscpf){
                self.fv[0].disableValidator('odin-cpf_cnpj', 'vat')
                .enableValidator('odin-cpf_cnpj', 'id')
                .revalidateField('odin-cpf_cnpj');        
              }
              else{
                self.fv[0].disableValidator('odin-cpf_cnpj', 'id')
    ...
    $('#odin-mainColor').minicolors({
      defaultValue: self.config.default.colors.mainColor !== undefined ? self.config.default.colors.mainColor : "#333",
      opacity:false,
  ...
  let form = document.getElementById(self.dfId);
    ...
    self.wizardActions(function(){ 
      ...
      document.getElementById(self.dfId).querySelector("[name='__dz_images']").value = JSON.stringify(self.dz.getOrderedDataImages());
      document.getElementById(self.dfId).querySelector("[name='__dz_copy_params']").value = JSON.stringify(self.dz.copy_params);
    ...
    self.callbacks.unload = self=>{
      $('#odin-cpf_cnpj, #odin-name, #odin-systemName, #odin-phone, #odin-mobile, #odin-email, #odin-pathStorage, #odin-address, #odin-address2, #odin-city, #odin-state').val('');
      $('#odin-fb-id, #odin-fb-version, #odin-fb-locale, #odin-fb-longToken').val('');
...
function view(self){
  return{
      onSuccess:function(data){
        ...
        $('#odin-cpf_cnpj').val(_conf.cpf_cnpj);

        if($('#odin-cpf_cnpj').cleanVal().length == 11){
          self.fv[0].disableValidator('odin-cpf_cnpj', 'vat')
          .enableValidator('odin-cpf_cnpj', 'id')
          .revalidateField('odin-cpf_cnpj');        
        }
...
function setCEP(data,self){
  ...
  if(self.toView !== null && $('#odin-zipCode').val() == _conf.zipCode){
    ...
    if(data!==null){
      //com logradouro   
      if(data.logradouro !==''){
         $('#odin-address').val(`${data.logradouro}${data.complemento!=''? ', '+data.complemento : ''}`);
      ...
      $('#odin-address').focus();
       ...
       $('#odin-address, #odin-address2, #odin-city, #odin-state').val('');
  }
