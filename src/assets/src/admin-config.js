new IOService({
    ...
    $('#admin-cpf_cnpj').mask($.jMaskGlobals.CPFCNPJMaskBehavior,{
      onKeyPress: function(val, e, field, options){
          ...
          args.push(iscpf=>{
            if(self.fv!==null){
              if(iscpf){
                self.fv[0].disableValidator('admin-cpf_cnpj', 'vat')
                .enableValidator('admin-cpf_cnpj', 'id')
                .revalidateField('admin-cpf_cnpj');        
              }
              else{
                self.fv[0].disableValidator('admin-cpf_cnpj', 'id')
  ...
  let form = document.getElementById(self.dfId);
                    ...
                    return new Promise(function(resolve, reject){
                      if(input.value.replace(/\D/g,'').length<8)
                        ...
                        $.ajax({
                          ...
                          url:`https://viacep.com.br/ws/${$('#admin-zipCode').cleanVal()}/json`,
                          success:(data)=>{
                            if(data.erro==true){
                              resolve({  
                                valid: false,
    ...
    self.callbacks.unload = self=>{
      $('#admin-cpf_cnpj, #admin-name, #admin-systemName, #admin-phone, #admin-mobile, #admin-email, #admin-pathStorage, #admin-address, #admin-address2, #admin-city, #admin-state').val('');
      $('#admin-fb-id, #admin-fb-version, #admin-fb-locale, #admin-fb-longToken').val('');
...
function setCEP(data,self){
  ...
  if(self.toView !== null && $('#admin-zipCode').val() == _conf.zipCode){
    ...
    if(data!==null){
      //com logradouro   
      if(data.logradouro !==''){
         $('#admin-address').val(`${data.logradouro}${data.complemento!=''? ', '+data.complemento : ''}`);
